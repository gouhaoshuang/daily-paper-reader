---
title: "Agentic Service-Oriented Computing: A Manifesto for the Next Frontier of Service-Oriented Computing"
title_zh: 智能体面向服务计算：面向服务计算下一个前沿的宣言
authors: "Amin Beheshti, Rong N. Chang, Boualem Benatallah, Fabio Casati, Schahram Dustdar, Geoffrey Fox, Quan Z. Sheng, Yan Wang, Jian Yang, Albert Zomaya"
date: 2026-07-14
pdf: "https://arxiv.org/pdf/2607.12619v1"
tags: ["query:eca"]
score: 6.0
evidence: 面向服务的分布式代理架构可应用于边缘云场景
tldr: LLM驱动的自主代理在分布式工作流中面临组合、互操作和治理等挑战，现有开发缺乏工程严谨性。为此提出面向代理的服务计算（ASOC），将代理工程化为服务，通过代理编排服务并治理生态系统。ASOC定义了六项基础原则（可驾驭性、可组合性、生命周期工程、可信设计、目标驱动编排、可观测与问责）和五维研究议程。借助服务计算社区二十余年积累，ASOC为代理AI从零散原型走向可靠、可信任的企业级系统提供工程脊梁。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2607.12619v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 913, \"height\": 708, \"label\": \"Figure\"}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2607.12619v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1883, \"height\": 393, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1878, \"height\": 914, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 889, \"height\": 430, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1883, \"height\": 437, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-005.webp\", \"caption\": \"\", \"page\": 0, \"index\": 5, \"width\": 922, \"height\": 588, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-006.webp\", \"caption\": \"\", \"page\": 0, \"index\": 6, \"width\": 1884, \"height\": 616, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2607.12619v1/table-007.webp\", \"caption\": \"\", \"page\": 0, \"index\": 7, \"width\": 947, \"height\": 911, \"label\": \"Table\"}]"
motivation: LLM驱动的自主代理在复杂分布式系统中缺乏工程严谨性，亟需服务计算社区的经验以确保可靠、可信的部署。
method: 提出ASOC框架，定义六项基础原则和五维研究议程，涵盖工程化、编排、治理、安全与信任等方面。
result: 建立了ASOC作为服务计算新前沿的概念基础与工程指导，促进代理AI向可信任系统演进。
conclusion: ASOC借助服务计算成熟方法，有望将代理AI转变为可靠、基于服务的系统。
---

## 摘要
由大型语言模型驱动的自主和半自主智能体的迅速涌现，正在将软件系统从静态的请求-响应组件重塑为目标导向、自适应且能使用工具的计算主体。随着这些智能体从孤立的认知原型发展为复杂的分布式工作流，它们面临着面向服务计算社区已研究二十余年的挑战：组合、互操作性、服务质量、生命周期管理、治理、安全性和信任。然而，当今许多智能体人工智能生态系统正在临时构建这些基础，缺乏可靠的企业和社会部署所需的工程严谨性。本文提出智能体面向服务计算（Agentic Service-Oriented Computing, ASOC）作为一个新的研究和实践领域，涉及将智能体作为服务进行工程化、通过自主和半自主智能体编排服务，并在信任、网络安全、合规性、性能和问责制的约束下治理智能体与服务生态系统。我们阐述了ASOC的六项基本原则（可用性、可组合性、生命周期工程、设计即可信、目标驱动编排、可观测性与问责制），并组织了一个涵盖五个维度的研究议程：（i）智能体服务基础与生命周期工程；（ii）组合、编排与互操作性；（iii）治理、可观测性与问责制；（iv）安全、信任与风险管理；以及（v）评估、认证与智能体服务质量。我们认为服务计算社区尤为适合为这一新兴领域提供概念和工程支撑，将智能体人工智能从零散示范转变为可靠的、基于服务的系统，值得人类和组织信任。

## Abstract
The rapid emergence of LLM-powered autonomous and semi-autonomous agents is reshaping software systems from static, request-response components into goal-directed, adaptive, and tool-using computational actors. As these agents move from isolated cognitive prototypes into complex distributed workflows, they confront challenges that the Service-Oriented Computing community has studied for more than two decades: composition, interoperability, quality of service, lifecycle management, governance, security, and trust. Yet much of today's agentic AI ecosystem is developing these foundations ad hoc, without the engineering rigour required for dependable enterprise and societal deployment. This paper introduces Agentic Service-Oriented Computing (ASOC) as a new research and practice area concerned with engineering agents as services, orchestrating services through autonomous and semi-autonomous agents, and governing ecosystems of agents and services under constraints of trust, cybersecurity, compliance, performance, and accountability. We articulate six foundational principles of ASOC (harness-ability, composability, lifecycle engineering, trustworthiness by design, goal-driven orchestration, and observability/accountability) and organise a five-dimensional research agenda spanning: (i) agentic services foundations and lifecycle engineering; (ii) composition, orchestration, and interoperability; (iii) governance, observability, and accountability; (iv) security, trust, and risk management; and (v) evaluation, certification, and Agentic QoS. We argue that the Services Computing community is especially well positioned to provide the conceptual and engineering spine for this emerging field, transforming agentic AI from fragmented demonstrations into dependable, service-based systems worthy of human and organisational trust.